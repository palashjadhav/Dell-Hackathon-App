...
def forecast():
    ...
    def train_and_predict():
        ...
        answer = regressor.predict(input_x)
        return answer,err
    clustering_method()
    ...
    sales = list()
    for i in range(12):
        s=m*(((a)**2)/p)*(2.71828**(-a*i))/((1+(f*2.71828**(-a*i)))**2)
        print(s)
        sales.append(s)
        sum=sum+s
    plt.plot(range(12) , sales, 'or-')
    ...
    max=ans+err
    min=ans-err
    session['Maximum']=round(max[0])
    session['Minimum']=round(min[0])
    results={
           "Min":round(min[0]),
           "Max":round(max[0])
        }
    ...
    if min<0:
        return render_template('next.html',max=round(max[0]),min=500.0)
    else:
        return render_template('next.html',err=err,max=round(max[0]),min=round(min[0]))
